...
import ThMod.cards.BlackOne.卍解;
import ThMod.cards.BlackOne.无月;
...
public class ThMod implements PostExhaustSubscriber,
  ...
  public void receiveEditCards() {
    ...
    BaseMod.addCard(new 月牙天冲());
    UnlockTracker.unlockCard(月牙天冲.ID);
    BaseMod.addCard(new 卍解());
    UnlockTracker.unlockCard(卍解.ID);
    BaseMod.addCard(new 无月());
    UnlockTracker.unlockCard(无月.ID);

    logger.info("add cards for MARISA");
    //starter:4
    BaseMod.addCard(new Strike_MRS());
    UnlockTracker.unlockCard(Strike_MRS.ID);
